...
<div>
  ...
  <div class="mb-4 grid grid-cols-2 flex-wrap items-center gap-5 md:gap-8 lg:flex">
    ...
    <x-secondary-button
      href="{{ route('admin.attendances.report', ['month' => $month, 'week' => $week, 'date' => $date, 'jobTitle' => $jobTitle, 'division' => $division]) }}"
        ...
        @foreach ($employees as $employee)
          <tr wire:key="{{ $employee->id }}" class="group">
            ...
            @if ($showUserDetail)
              ...
              <td class="{{ $class }} text-nowrap group-hover:bg-gray-100 dark:group-hover:bg-gray-700">
                @if ($isPerDayFilter && $employee->attendance)
                  {{ $employee->attendance->shift?->name ?? '-' }}
                @else
                  -
                @endif
              </td>
              @php
                ...
                $timeIn = $attendance ? ($attendance->time_in ? Carbon::parse($attendance->time_in)->format('H:i') : null) : null;
                $timeOut = $attendance ? ($attendance->time_out ? Carbon::parse($attendance->time_out)->format('H:i') : null) : null;
                $isPresent = $attendance && ($attendance->status == 'present' || $attendance->status == 'late');
              @endphp
              @if (!$isPerDayFilter)
                <td class="{{ $class }} text-nowrap group-hover:bg-gray-100 dark:group-hover:bg-gray-700">
                  @if($isPresent)
                    <span class="px-2 py-1 rounded bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200">
                  ...
                  @else
                    <span class="px-2 py-1 rounded bg-red-200 dark:bg-red-800 text-red-800 dark:text-red-200">Tidak Hadir</span>
                  @endif
                </td>
              @endif
            ...
            @foreach ($dates as $date)
              @php
                ...
                if ($isPerDayFilter) {
                    ...
                    $attendance = $employee->attendances->first(function($item) use ($date) {
              ...
              <td
                ...
                @if($attendance) wire:click="showAttendanceDetail({{ $attendance->id ?? 'null' }})" @endif>
                {{ $displayStatus }}
              </td>
            @endforeach
            ...
            @if ($isPerDayFilter)
              ...
              <td
                ...
                @if($attendance)
                  <button wire:click="showAttendanceDetail({{ $attendance->id }})" class="text-blue-500 hover:text-blue-700">
